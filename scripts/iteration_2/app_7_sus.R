...
server <- function(input, output) {
  ...
  res<-eventReactive(input$go,{
    a<-mcmapply(FUN = exposure,logE,p,as.numeric(input$ach),as.numeric(input$times),as.numeric(input$delay),mc.cores = 1)%>%t()%>%as.data.frame() %>% unnest(cols=c(C1, C2, C3, C4, C5, C6, C7, C8, dose, risk))%>%pivot_longer(!c(dose,risk))
    return(a)
  })
  ...
  output$violinPlot<-renderPlot({
    ...
    res()%>%
      ...
      scale_x_discrete(labels= c("Infectious Patient","Susceptible patient 1", "Susceptible patient 2","Susceptible patient 3","Susceptible patient 4","Susceptible patient 5","Susceptible patient 6","Susceptible patient 7"))+
      xlab("")+
      labs(
