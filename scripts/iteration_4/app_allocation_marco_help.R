...
patient.time.in <- df[["Appointment.Start.time"]]
...
patient.date.in <- df[["Appointment.start.date"]]

appointment.length <- df[["Appointment.length"]]

#appointment.length <- c(runif(n=NROW(patient.time.in),min=10,max=90))

patient.infection <-df[["Infection.type"]]
...
patient.roster <- patient.roster %>%
  ungroup() %>% 
  mutate(clinician=rep(c("Ian","Dan"),6)) %>% 
  group_by(clinician) %>% 
