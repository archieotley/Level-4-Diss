...
server <- function(input, output) {
  ...
  exposure<-function(logE,p,ach,times,delay){
    ...
    #AGO#########################################################################
    #AGO##################################### Part 10 susceptible patient enters room and breaths in CFU
    
    #Intial conditions
    #E0=0
    t10=tend9
    tend10= tend9 +as.numeric(input$times)*60 #X * 60 X is time in mins
    #tspan2=[t02 tend2]
    times <- seq(t10, tend10, by = 1)
    #C02=C1(end)    #takes the last concentration value form simulation 1
    #E0=0           #emission now 0
    
    parameters<-c(0,V,lambda)
    init<-c(C=out9$C[NROW(out9)])
    #out3 <- deSolve::lsoda(init, times = times, func = numerical_ODE, parms = parameters)%>% as_tibble()
    
    analyticalODE(times,0,V,lambda,out8$C[NROW(out9)])-> out10
  ...
  res<-eventReactive(input$go,{
    a<-mcmapply(FUN = exposure,logE,p,as.numeric(input$ach),as.numeric(input$times),as.numeric(input$delay),mc.cores = 1)%>%t()%>%as.data.frame() %>% unnest(cols=c(C1, C2, C3, C4, C5, C6, C7, C8, C9, C10, dose, risk))%>%pivot_longer(!c(dose,risk))
    return(a)
  })
  ...
  output$violinPlot<-renderPlot({
    ...
    res()%>%
      ...
      scale_x_discrete(labels= c("Infectious Patient","Susceptible patient 1", "Susceptible patient 2","Susceptible patient 3","Susceptible patient 4","Susceptible patient 5","Susceptible patient 6","Susceptible patient 7","Susceptible patient 8","Susceptible pateint 9"))+
      xlab("")+
      labs(
  ...
  w <- reactive({
    a<-res() %>%
      filter(name=="C10")%>% #AGO (changed C1 to C4) Choose any patient otherwise you're triplicating the subsequent calculations
      select(risk) %>%
