...
half_life = 30+runif(N, min=0,max=121)
...
##### cheat way to get risk wanted for each person
Np=N/4
timesa<-runif(Np,min = 10,max = 120)
timesb<-runif(Np,min = 10,max = 120)
timesc<-runif(Np,min = 10,max = 120)
timesd<-runif(Np,min = 10,max = 120)
pa = ((CI[1]+runif(Np)*CI[2]+runif(Np))*0.001)/60 
pb = ((CI[1]+runif(Np)*CI[2]+runif(Np))*0.001)/60 
pc = ((CI[1]+runif(Np)*CI[2]+runif(Np))*0.001)/60 
pd = ((CI[1]+runif(Np)*CI[2]+runif(Np))*0.001)/60 
ka=runif(Np,min = 1e-4,max = 1e-3)
kb=runif(Np,min = 1e-4,max = 1e-3)
kc=runif(Np,min = 1e-4,max = 1e-3)
kd=runif(Np,min = 1e-4,max = 1e-3)
...
# Exposure function -----------------------------------------------------
exposure<-function(logE,p,ach,times,delay,k,V,lambda){## alter times,k, and p so that they match the values calculated with, so plot will work
  ...
  tmp<-
    ...
    #Note the multiplication instead of division, this is because of how the dose-response parameter is given
 # WON'T WORK NEED CORESPONDING VALUES FOR IT TO CALCULATE THE GRAOH CORRECTLY'
  # summarise(dose1=AUC(timesa,(C1)*pa))%>%
  #   mutate(risk1=1-exp(-dose1*as.numeric(ka)))
  # 
  # summarise(dose2=AUC(timesb,(C2)*pb))%>%
  #   mutate(risk2=1-exp(-dose2*as.numeric(kb)))
  # 
  # risk->risk1+risk2
...
df=mcmapply(FUN = exposure,logE,p,ach,times,delay,k,V,lambda,mc.cores = 1) %>%
  t()%>%
  as.data.frame() %>%
  unnest(cols=c(C1, C2, dose, risk))#unnest(cols=c(C1, C2, C3, dose, risk))
...
fit.lm=lm(data=df %>% select(-c(C1,C2,risk)),log10(dose)~.)#select(-c(C1,C2,C3,risk)),log10(dose)~.)
plot(fit.lm)
