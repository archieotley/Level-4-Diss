...
exposure<-function(logE,p,ach,times,delay,k,V,lambda){
  ...
  tmp3<-
    out3%>%
    summarise(dose3=AUC(times,C*p))%>%
    mutate(risk=1-exp(-dose3*as.numeric(k))) #Note the multiplication instead of division, this is because of how the dose-response parameter is given
  
  tmp5<-
    out5%>%
    summarise(dose5=AUC(times,C*p))%>%
    mutate(risk5=1-exp(-dose5*as.numeric(k))) #Note the multiplication instead of division, this is because of how the dose-response parameter is given
...
df=mcmapply(FUN = exposure,logE,p,ach,times_appt,delay,k,V,lambda,mc.cores = 1) %>%
  t()%>%
  as.data.frame() %>%
  unnest(cols=c(C1, C2, C3,C4,C5, dose3, risk3))#,dose5,risk5))
#pivot_longer(!c(dose,risk))


df=cbind(df,logE,p,ach,times_appt,delay,k,V,lambda)
...
fit.lm=lm(data=df %>% select(-c(C1, C2, C3,C4,C5, risk3)),log10(dose)~.)
plot(fit.lm)
