...
exposure<-function(logE,p,ach,times,delay,k,V,lambda){
  ...
  #E0=0
  t06=tend5
  tend6= tend5 +as.numeric(input$delay)*60 #X * 60 X is time in mins
  #tspan2=[t02 tend2]
  ...
  #E0=0
  t08=tend7
  tend8= tend7 +as.numeric(input$delay)*60 #X * 60 X is time in mins
  #tspan2=[t02 tend2]
...
df=mcmapply(FUN = exposure,logE,p,ach,times,delay,k,V,lambda,mc.cores = 1) %>%
  t()%>%
  as.data.frame() %>%
  unnest(cols=c(C1, C2, C3, C4, C5, C6, C7,C8,dose, risk))
...
fit.lm=lm(data=df %>% select(-c(C1,C2,C3,C4,C5,C6,C7,C8,risk)),log10(dose)~.)
plot(fit.lm)
